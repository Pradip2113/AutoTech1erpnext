...
class PurchaseReceipt(BuyingController):
	...
	def before_save(self):
		for d in self.get('items'):
			if d.schedule_date and d.purchase_order:
				posting_date =  datetime.strptime(self.posting_date, '%Y-%m-%d').date()
				schedule_date = datetime.strptime(d.schedule_date, '%Y-%m-%d').date()

				ten_days_before_posting = schedule_date - timedelta(days=10)
				ten_days_after_posting = schedule_date + timedelta(days=10)

				# x = frappe.utils.subtract_days(schedule_date, 10)
				# y = frappe.utils.add_days(schedule_date, 10)
				# frappe.throw(str(x)+"===="+str(y))

				# frappe.throw(str(ten_days_before_posting)+"===="+str(schedule_date)+"===="+str(ten_days_after_posting))
				if not (ten_days_before_posting <= posting_date <= ten_days_after_posting):
					d.check_of_required_by_date_ = True
					
					
		for d in self.get('items'):
			if d.check_of_required_by_date_:
				frappe.throw(f'Required By date for item {d.item_code} is between {ten_days_before_posting} and {ten_days_after_posting}')			
