...
class ProductionPlan(Document):
	...
	def make_work_order(self):
		...
		# self.check_work_order=2
		temp=''
		qty=0
		quant=0
		count=1.0
		for i in self.get('sales_orders'):
			temp=i.sales_order
			
			so=frappe.get_list('Sales Order',fields=["name","total_qty"],filters={"name":temp})
			# frappe.msgprint(str(so))
			for j in so:
				qty+=j.total_qty

			wo=frappe.get_list('Work Order',fields=["name","sales_order","qty","production_plan"],filters={"sales_order":temp})
			# frappe.msgprint(str(wo))
			for k in wo:
				quant+=k.qty
				count+=1.0
		# frappe.msgprint(str(quant)+'    '+str(qty))
		if count<=qty and quant<qty:
			...
			frappe.msgprint('Cannot produce more work order than Sales Order quantity')
