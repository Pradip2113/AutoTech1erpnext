...
# 				frappe.msgprint("ajhfgsdfh")
# 				for item in self.get("items"):
# 					if item.get("name") not in self.get("__removed_items"):
# 						frappe.msgprint(_("Item No: {0} has been removed.").format(item.get("item_code")))
# # ---------------------------------------------------------------------------------------------------------------		
...
class SalesOrder(SellingController):
	...
	def before_submit(self):
		self.Minimum_item_material_request()
		self.set("temp_table", [])
		list_1 = []
		list_1.clear()
	...
	# ------------------------------------------------------------------------------------------------------------------------------------------------
	@frappe.whitelist()
	def Minimum_item_material_request(self):
		for i in self.get('items'):
			valable_qty = (frappe.get_all('Bin', filters={'item_code': i.item_code, 'warehouse': self.set_warehouse}, fields=['SUM(actual_qty) as qty']))[0]['qty']
			minimum_qty = frappe.get_all('Item Reorder', filters={'parent': i.item_code, 'warehouse' : self.set_warehouse }, fields =['warehouse_reorder_level','warehouse_reorder_qty'])
			# frappe.throw(str( valable_qty )+'======'+str(minimum_qty[0]['warehouse_reorder_level']))
			if minimum_qty:
				if valable_qty <= minimum_qty[0]['warehouse_reorder_level']:
					mr_doc = frappe.new_doc("Material Request")
					mr_doc.set_warehouse = self.set_warehouse
					mr_doc.append(
							"items",
							{
					...
					notification_log.for_user = frappe.session.user
					notification_log.creation = now_datetime()
					notification_log.modified = now_datetime()
